...
domain_name = '0a75-220-130-85-186.ngrok.io'
...
app.config['SQLALCHEMY_DATABASE_URI'] = f"mssql+pymssql://{web_config.testing().username}:{quote(web_config.testing().password)}@{web_config.testing().server}/{web_config.testing().database}"
db.init_app(app)
...
def handle_postback(event):
    ...
    if payment:
        ...
        if UserMemberInfo['UserId'] != None:
            ...
            if isSuccess:
                line_bot_api.reply_message(event.reply_token, TextSendMessage(text='成功接受挑戰!\n'+isSuccess))
        else:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='請先登入！'))
def set_UserMemberInfo(event, UserMemberInfo):
    ...
    FlexMessage['body']['contents'][0]['contents'][0]['contents'][2]['text'] = f"訂閱到期時間 : {UserMemberInfo['SubscribeEnd_dd'].strftime('%Y-%m-%d %H:%M')}"
    # 已付款(2022/8/5)
    FlexMessage['body']['contents'][0]['contents'][0]['contents'][3]['text'] = f"是否付款 : {UserMemberInfo['isPayment']} ({UserMemberInfo['Payment_dd'].strftime('%Y-%m-%d %H:%M')})"
...
def update_invite_LinePlayerPK(UserId, data, event):
    ...
    try:
        ...
        if result['Option2'] == None and result['UserId2'] != None and result['MatchTime'].replace(tzinfo=utc) >= datetime.now().astimezone(timezone(timedelta(hours=8))).replace(tzinfo=utc):
            ...
            message = f"[場次 {data['HomeTeam']} VS {data['AwayTeam']}，盤口 {data['GroupOptionName']}]"
            line_bot_api.push_message(data['LineUniqueID'],TextSendMessage(f"{get_member(UserId)}已接受您的邀請\n"+message))
            return message
        else:
